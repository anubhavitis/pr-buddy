...
test("completeRequest uses the host JSON field names", () => {
  ...
  assert.deepEqual(Object.keys(body).sort(), ["backend", "mlx_model", "mlx_url", "model", "prompt"]);
...
test("completeRequest sends the claude model when set", () => {
  const body = completeRequest(
    { ...defaultSettings, backend: "claude", claudeModel: "opus" },
    "order these files",
  );
  assert.equal(body.model, "opus");
  assert.equal(body.mlx_model, "");
});
